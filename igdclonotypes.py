...
# Copied from IgDiscover 0.15 (src/igdiscover/cli/clonotypes.py) and modified.
#
# Changes:
# - Runs standalone
# - Added a --clustered command-line option for outputting a different type of
#   table. It contains the same information as what the --members table,
#   but in a more easily usable format. It is also faster than --members.
# - The command was sped up a little bit for the case when not using --clustered
# - Added the ability to accept multiple input tables. They are merged and then
...
from igdiscover.cluster import hamming_single_linkage
from igdiscover.utils import slice_arg, nt_to_aa
...
def main():
    ...
    arg("--members", metavar="FILE", help="Write member table to FILE")
    arg(
        "--clustered",
        metavar="FILE",
...
def run_clonotypes(
    ...
    table.insert(
        list(table.columns).index("cdr3"), "CDR3_length", table["cdr3"].apply(len)
    )
    ...
    if mindiffrate:
        barcode_col_index = table.columns.get_loc(
            "barcode"
        ...
        if "VDJ_nt" not in columns:
            ...
            def extract_vdj_nt(row):
                if row["v_call"] and row["j_call"]:
                    return row["sequence"][
                        row["v_sequence_start"] - 1 : row["j_sequence_end"]
                    ]
                return None
    ...
    with ExitStack() as stack:
        ...
        for group in itertools.islice(grouped, 0, limit):
            ...
            if members_file:
                ...
                print(
                    group.drop("clonotype_id", axis=1).to_csv(
                        sep="\t", header=members_header, index=False
                    ),
                    file=members_file,
                )
...
def add_clonotype_id(
    table: pd.DataFrame, mismatches: int, cdr3_column: str, cdr3_core
) -> pd.DataFrame:
    """
    Cluster sequences into clonotypes and add a 'clonotype_id' column
    """
    table["vjlen_id"] = table.groupby(
        ["v_call", "j_call", "CDR3_length"], sort=False
    ).ngroup()
...
def is_similar_with_junction(s, t, mismatches, cdr3_core):
    ...
    *and* have at least one identical junction.
    ...
    return distance_ok and (
        (s[: cdr3_core.start] == t[: cdr3_core.start])
        or (s[cdr3_core.stop :] == t[cdr3_core.stop :])
    )
...
def augment_group(table, v_shm_threshold=5, suffix="_mindiffrate"):
    ...
    for column in ["cdr3", "cdr3_aa", "VDJ_nt", "VDJ_aa"]:
        ...
        table[column + suffix] = table[column].apply(
            lambda s: round(
